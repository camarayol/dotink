...
local function create_directory(dir)
    ...
    if parent and parent:len() > 0 then
        if create_directory(parent) then
            local ret, err = lfs.mkdir(dir)
            if not ret then
                logger.error("mkdir %s failed. %s", dir, err)
                return false
            end
            return true
        end
    end
    logger.error("mkdir %s failed.", dir)
    return false
end

local function create_symbolic_link(origin, target)
    if opts.create_directory then
        create_directory(parentdir(target))
    end
